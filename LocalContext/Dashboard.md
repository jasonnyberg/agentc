...
**Current State**: G111 is complete; G091 has first retained-terminal/drop/abandoned/non-retroactive-cancel lifecycle semantics, G099 has first opt-in Edict task/status validators, and G110 now has known-resource abandoned-owner recovery, deterministic lease/expiry recovery plus participant heartbeat renewal and pidfd owner-death recovery for registered resources, logical Root1 resource/lease wrappers in `root1.edict`, and non-parking module-backed `root1.await!`.

**Next Action**: Continue 🔗[G110](./Knowledge/Goals/G110-EventfdEpollMicroVmIpcDesign/index.md) by designing the continuation-parking `await!` scheduler boundary: decide where parked continuations live, how descriptor readiness resumes them, and what minimal test-only VM scheduler hook is needed.
