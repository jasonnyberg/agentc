...
- **Current Task**: Continue the runtime extraction after landing automated coverage and the first physical source-layout migration, then start connecting the runtime-backed host to the mmap/Listree persistence lifecycle.
- **Immediate Next Action**: Migrate the remaining reusable runtime support code toward `cpp-agent/runtime/`, then begin wiring checkpoint/restore hooks for the runtime-backed host over the embedded VM/slab substrate.
...
- Automated validation is now in place: `cpp_agent_tests` covers both the raw runtime C ABI and the Edict-side wrapper/module path, passes under direct execution and `ctest -R cpp_agent_tests`, and verifies normalized error-envelope behavior through both layers.
- Physical source migration has started in earnest: the active `agent_runtime` target now builds from `cpp-agent/runtime/common/{credentials,http_client,sse_parser}.*` and `cpp-agent/runtime/providers/{google,openai}/...` instead of the older top-level source locations.
...
## Handoff Note
The project is evolving toward an Edict-native agent runtime: keep the reconnectable Client/Host boundary and embedded VM, but move the canonical agent loop into Edict and expose LLM access as an importable `agentc` module backed by a reusable native runtime library. The runtime bridge is now working end-to-end and has automated coverage: `libagent_runtime.so` builds from the new runtime scaffolding, `cpp-agent/main.cpp` is a thinner runtime-backed socket host with clean `shutdown-agent` behavior and JSON-configurable runtime selection, `cpp-agent/edict/modules/agentc.edict` proves the Edict-side import/wrapper path, and `cpp_agent_tests` validates both the raw C ABI and Edict wrapper/module path under CTest. The exact next action is to continue migrating the remaining reusable runtime support code into `cpp-agent/runtime/`, then start wiring the runtime-backed host into the mmap/Listree persistence lifecycle so G068 begins converging with the slab-backed restore architecture. Key context: keep Client/Host separation, keep Host+VM embedded, keep mmap-backed Listree/slab persistence as the durable substrate, treat provider/runtime handles as transient rehydrated state, and keep unrestricted model-driven FFI/native access disabled by default. Do NOT reintroduce host-owned orchestration as the long-term architecture; the remaining outer-host logic is transitional and should keep shrinking.
