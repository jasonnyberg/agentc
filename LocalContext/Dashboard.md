...
- **Current Task**: Continue the runtime extraction after landing transcript-level Listree-backed persistence hooks in the runtime-backed host, then push the persistence boundary deeper toward embedded VM/root-anchor ownership.
- **Immediate Next Action**: Migrate the remaining reusable runtime support code toward `cpp-agent/runtime/`, then advance persistence from transcript-level restore to embedded VM/root-anchor restore in the host lifecycle.
