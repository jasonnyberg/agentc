...
- **Current Task**: Turn the runtime architecture into an Edict-native loop/module plan: `agentc` as an importable Edict capability backed by a reusable native LLM runtime library, with the host reduced to transport/lifecycle duties.
- **Immediate Next Action**: Write the concrete `libagent_runtime` C ABI and the normalized `agentc.call` request/response schema, then map the current `cpp-agent` provider/auth/http code into that extraction plan.
...
## Handoff Note
The project is evolving toward an Edict-native agent runtime: keep the reconnectable Client/Host boundary and embedded VM, but move the canonical agent loop into Edict and expose LLM access as an importable `agentc` module backed by a reusable native runtime library. LMDB cleanup is complete, and the new architecture plan is captured in 🔗[WP_EdictNativeAgentModuleArchitecture](./Knowledge/WorkProducts/WP_EdictNativeAgentModuleArchitecture.md) plus the updated 🔗[G068](./Knowledge/Goals/G068-ClientAgentSplitEmbeddedVmPersistence/index.md). The exact next action is to write the concrete `libagent_runtime` C ABI and normalized `agentc.call` request/response schema, then map current `cpp-agent` provider/auth/http code into that extraction plan. Key context: keep Client/Host separation, keep Host+VM embedded, treat provider/runtime handles as transient rehydrated state, and keep unrestricted model-driven FFI/native access disabled by default. Do NOT preserve the outer C++ agent loop as the long-term owner of policy/orchestration; it is now a transitional prototype, not the target architecture.
