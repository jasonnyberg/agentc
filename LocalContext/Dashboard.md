...
3. 🔗[G111 — Root1/Worker Primitive FFI and Edict Intern Surface Migration](./Knowledge/Goals/G111-Root1WorkerPrimitiveFfiEdictInternMigration/index.md) — **PLANNED / NEXT**; migrate `intern_run!`, `intern_start!`, and `intern_sync!` from intern-specific VM opcodes to importable Root1/worker primitives plus plain Edict policy words.
...
**Next Action**: Start 🔗[G111](./Knowledge/Goals/G111-Root1WorkerPrimitiveFfiEdictInternMigration/index.md) with a behavior-freezing/extraction slice: preserve current `InternWorkerTest.*` expectations, extract the native worker/broker services behind importable Root1/worker primitives, then rebuild `intern_run!`, `intern_start!`, `intern_sync!`, and `intern_cancel!` as Edict module words. After that, close remaining 🔗[G091](./Knowledge/Goals/G091-InternWorkerConcurrencyMvp/index.md) lifecycle cleanup and promote 🔗[G099](./Knowledge/Goals/G099-InternTaskQualityContracts/index.md) using the Edict-level contract helpers. Keep 🔗[G110](./Knowledge/Goals/G110-EventfdEpollMicroVmIpcDesign/index.md) open for abandoned-resource recovery and future `await!` semantics.
