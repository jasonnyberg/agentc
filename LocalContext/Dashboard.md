...
**Current State**: G111 is complete; G091 has first retained-terminal/drop/abandoned/non-retroactive-cancel lifecycle semantics, G099 has first opt-in Edict task/status validators, and G110 now has known-resource abandoned-owner recovery, deterministic lease/expiry recovery plus participant heartbeat renewal and pidfd owner-death recovery for registered resources, and non-parking module-backed `root1.await!`.

**Next Action**: Continue 🔗[G110](./Knowledge/Goals/G110-EventfdEpollMicroVmIpcDesign/index.md) by deciding whether lease registration/heartbeat/recovery should be exposed through `agentc_root1_primitives.h` / `root1.edict` now, or remain C++ substrate until process-worker ownership needs it; if exposing, add LTV wrappers and module tests.
...
- Current G110 abandoned recovery has three layers: explicit `recoverAbandonedResource(key, state, owner, reason)`, lease-driven `registerLease` / `renewLease` / `recoverExpiredLeases(nowTick, reason)`, and participant-level `heartbeatParticipant` / `recoverParticipantLeases` used by pidfd owner-death handling.
