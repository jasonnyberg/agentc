...
- **Active Goals**:
  ...
  - **G070** — Inverted Loop Surgical Cleanup — **PROPOSED** 🔗[index](./Knowledge/Goals/G070-InvertedLoopSurgicalCleanup/index.md)
  - **G069** — Remove LMDB Dependencies — **COMPLETE** 🔗[index](./Knowledge/Goals/G069-RemoveLMDBDependencies/index.md)
- **Current Task**: Use the new G070 cleanup plan to drive the next implementation slice: pin down the production ownership boundary around embedded VM restore and host turn handling.
- **Immediate Next Action**: Start G070 Phase 1 by mapping the current production turn path and transient runtime/import artifact set, then use that map to move one turn/state transition boundary inward.
...
## Handoff Note
The project is evolving toward an Edict-native agent runtime: keep the reconnectable Client/Host boundary and embedded VM, but move the canonical agent loop into Edict and expose LLM access as an importable `agentc` module backed by a reusable native runtime library. The runtime bridge is now working end-to-end, has automated coverage, includes a deeper persistence hook, and now has hello-world, multi-turn history, canonical agent-root, host-root-persistence, embedded-VM/root-anchor, and operator-config slices: `libagent_runtime.so` builds from the new runtime scaffolding, `cpp-agent/main.cpp` is a thinner runtime-backed socket host with clean `shutdown-agent` and `reset-session` behavior, `cpp-agent/edict/modules/agentc.edict` now supports JSON + file/path-based config wrappers using Edict-native `read_text` + `from_json`, `agentc-config.json` is the new operator-facing runtime config, `cpp_agent_tests` validates the raw C ABI + Edict wrapper + root helpers + persistence path, `cpp-agent/runtime/persistence/session_state_store.*` exposes both JSON and native root-anchor restore/save helpers, `cpp-agent/runtime/persistence/agent_root_state.*` shapes the canonical root object, and `cpp-agent/main.cpp` constructs an embedded `EdictVM` around the restored anchored root. Live config-driven demos now work without recompilation, and the runtime persistence demo again shows restart continuity under `gemini-2.5-flash`. The next execution slice is now explicitly captured in 🔗[G070](./Knowledge/Goals/G070-InvertedLoopSurgicalCleanup/index.md): make the VM the real owner of turn/state transitions, define the transient rehydration boundary for runtime handles/imports around embedded VM restore, and remove JSON rematerialization from the normal native-root persistence path. Key context: keep Client/Host separation, keep Host+VM embedded, keep mmap/Listree/slab persistence as the durable substrate, treat provider/runtime handles as transient rehydrated state, and keep unrestricted model-driven FFI/native access disabled by default. Do NOT reintroduce host-owned orchestration as the long-term architecture; the remaining outer-host logic is transitional and should keep shrinking.
