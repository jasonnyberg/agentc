...
**Current State**: G111 is complete; G091 has first retained-terminal/drop/abandoned/non-retroactive-cancel lifecycle semantics, G099 has first opt-in Edict task/status validators, and G110 now has known-resource abandoned-owner recovery, deterministic lease/expiry recovery plus participant heartbeat renewal and pidfd owner-death recovery for registered resources, logical Root1 resource/lease wrappers in `root1.edict`, non-parking module-backed `root1.await!`, `EdictVM::resume()` for yielded code frames, and a first in-process `Root1AwaitScheduler` parking/resume table.

**Next Action**: Continue 🔗[G110](./Knowledge/Goals/G110-EventfdEpollMicroVmIpcDesign/index.md) by replacing the test-only raw `EdictVM*` parking record in `edict/root1_await_scheduler.{h,cpp}` with a logical continuation handle/status surface that can report parked/resumed/timeout/cancelled states without exposing VM pointers.
