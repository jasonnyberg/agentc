...
3. 🔗[G111 — Root1/Worker Primitive FFI and Edict Intern Surface Migration](./Knowledge/Goals/G111-Root1WorkerPrimitiveFfiEdictInternMigration/index.md) — **ACTIVE / NEXT**; first slices added importable LTV worker primitives plus `worker.edict` / `intern.edict`, module-backed intern words compose prepare/capacity/drain/collect-status/request-cancel/drop pieces, public backpressure and async sync/cancel envelope construction have moved into `intern.edict`, first generic `root1.edict` participant/poll/mailbox wrappers are importable, and intern VM opcode shims have been deleted; remaining work is start/run envelope migration, lifecycle cleanup, and splitting/cleaning the native primitive implementation.
...
**Next Action**: Continue 🔗[G111](./Knowledge/Goals/G111-Root1WorkerPrimitiveFfiEdictInternMigration/index.md) after deleting the intern opcode shims: migrate or explicitly classify remaining start/run envelope shape, split/clean the native worker primitive implementation, then close remaining 🔗[G091](./Knowledge/Goals/G091-InternWorkerConcurrencyMvp/index.md) lifecycle cleanup and promote 🔗[G099](./Knowledge/Goals/G099-InternTaskQualityContracts/index.md) using the Edict-level contract helpers. Keep 🔗[G110](./Knowledge/Goals/G110-EventfdEpollMicroVmIpcDesign/index.md) open for abandoned-resource recovery and future `await!` semantics.
...
- 🔗[G111 — Root1/Worker Primitive FFI and Edict Intern Surface Migration](./Knowledge/Goals/G111-Root1WorkerPrimitiveFfiEdictInternMigration/index.md) — active migration from intern VM opcodes to importable native Root1/worker primitives plus Edict module policy.
