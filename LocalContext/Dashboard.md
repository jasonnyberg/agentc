...
Completed implementation/documentation slices are retired from the open-dashboard view; the incomplete backlog is now listed below in priority order. 🔗[G080 — LLM REPL Context Management](./Knowledge/Goals/G080-LlmReplContextManagement/index.md) is complete, so the next implementation candidate under the active parent track 🔗[G078 — Edict-Resident Agent Loop Consolidation](./Knowledge/Goals/G078-EdictResidentAgentLoopConsolidation/index.md) is 🔗[G091 — Intern Worker Concurrency MVP](./Knowledge/Goals/G091-InternWorkerConcurrencyMvp/index.md).
...
2. 🔗[G091 — Intern Worker Concurrency MVP](./Knowledge/Goals/G091-InternWorkerConcurrencyMvp/index.md) — **NEXT**; coordinator/worker VMs with private slabs, read-only shared context/imports, and structured result merge.
...
**Current State**: The foundational persistence/client-host work has been retired to the archive. The live project is now centered on G078: making Edict the authoritative control plane for the agent loop. The codebase has a working curated launcher, Edict-side provider objects, provider REPL, local/Codex/Google-Gemma provider paths, provider-owned reset/inspect context management, first file/shell tool surface, first real-time ghost-queue stream surface, strict lookup modes, corrected prefix-chain semantics, and corrected tail-history semantics.

**Next Action**: Start G091. Build the first intern-worker concurrency MVP: coordinator/worker VM boundaries, private slabs, read-only shared context/imports, and structured result merge. Keep G099 task-quality contracts close behind as the companion policy layer.
...
- 🔗[G091 — Intern Worker Concurrency MVP](./Knowledge/Goals/G091-InternWorkerConcurrencyMvp/index.md) — next local-intern worker architecture slice.
...
- [x] Completed G080 provider-owned LLM REPL context reset/inspect slice and promoted G091 to next implementation candidate
- [x] Updated Dashboard
- [x] Updated Timeline
