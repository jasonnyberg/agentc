...
2. 🔗[G110 — Root1 eventfd/epoll Resource Broker and Micro-VM IPC Design](./Knowledge/Goals/G110-EventfdEpollMicroVmIpcDesign/index.md) — **ACTIVE**; first Linux prototype landed with participant eventfds, Root1 epoll polling, resource keys, ownership-requested flags, wait queues, grant tokens, fixed-size mailbox descriptors, and a bounded mailbox ring; next is full mmap coordination slab placement plus fd reconstruction/owner-death semantics.
...
- **Validation baseline from latest implementation pass**: G110 prototype validation passed with `cmake --build build --target reflect_tests -j2`, `./build/tests/reflect_tests --gtest_filter='Root1ResourceBrokerTest.*'` 4/4, full `./build/tests/reflect_tests` 37/37, `cmake --build build --target edict_tests -j2`, and `cmake --build build --target cpp_agent_tests -j2`. Earlier baseline: `InternWorkerTest.*` passed 2/2 for the G091 first slice; focused pop-transition tests passed 19/19; focused `edict_tests` style/regression slice passed 59/59 including `PiSimulationTest.MiniKanrenLogicExample`; focused cpp-agent Edict/LLM suite passes 21/21 including live Google/Gemma coverage and G080 REPL context management.
...
**Next Action**: Continue 🔗[G110](./Knowledge/Goals/G110-EventfdEpollMicroVmIpcDesign/index.md) by turning the in-process sidecar prototype and bounded ring into a first explicit mmap coordination slab placement design: descriptor/ring placement, resource-state word placement, participant mailbox records, fd reconstruction on resume, and pidfd/owner-death recovery. Then continue 🔗[G091](./Knowledge/Goals/G091-InternWorkerConcurrencyMvp/index.md) with broker-compatible `intern_start!` / `intern_sync!`; treat 🔗[G109](./Knowledge/Goals/G109-ListreeReadOnlyMutationSurfaceHardening/index.md) as the immediate safety companion before trusting shared read-only worker context.
