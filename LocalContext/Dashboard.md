...
4. 🔗[G099 — Intern Task Quality Contracts](./Knowledge/Goals/G099-InternTaskQualityContracts/index.md) — **ACTIVE / NEXT**; minimal bounded/checkable async intern task schemas now have first Edict-level task/status validators; next is dispatch integration and malformed/over-broad task rejection.
...
**Current State**: G111 is complete; G091 has retained-terminal/drop/abandoned/non-retroactive-cancel lifecycle semantics plus first `yield!`-based cooperative cancellation checkpoints, G099 has first opt-in Edict task/status validators, and G110 now has known-resource abandoned-owner recovery, deterministic lease/expiry recovery plus participant heartbeat renewal and pidfd owner-death recovery for registered resources, logical Root1 resource/lease wrappers in `root1.edict`, non-parking module-backed `root1.await!`, `EdictVM::resume()` for yielded code frames, a first in-process `Root1AwaitScheduler` parking/resume table with logical continuation handles/status, and a documented public parking `await!` contract boundary.

**Next Action**: Continue 🔗[G099](./Knowledge/Goals/G099-InternTaskQualityContracts/index.md) by deciding where `intern_start!` should enforce `intern.validate_task_contract!` and adding malformed/over-broad task rejection tests.
