...
3. 🔗[G111 — Root1/Worker Primitive FFI and Edict Intern Surface Migration](./Knowledge/Goals/G111-Root1WorkerPrimitiveFfiEdictInternMigration/index.md) — **ACTIVE / NEXT**; first slices added importable LTV worker primitives plus `worker.edict` / `intern.edict`, module-backed intern words compose prepare/capacity/drain/collect/request-cancel/drop pieces, public backpressure envelope construction has moved into `intern.edict`, and first generic `root1.edict` participant/poll/mailbox wrappers are importable; remaining work is to move running/final/unknown-job envelope construction and broader waitable policy into Edict before deleting opcode shims.
