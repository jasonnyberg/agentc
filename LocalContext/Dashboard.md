...
2. 🔗[G110 — Root1 eventfd/epoll Resource Broker and Micro-VM IPC Design](./Knowledge/Goals/G110-EventfdEpollMicroVmIpcDesign/index.md) — **ACTIVE**; first Linux prototype landed with participant eventfds, Root1 epoll polling, resource keys, ownership-requested flags, wait queues, grant tokens, fixed-size mailbox descriptors, bounded mailbox ring, and file-backed `CoordinationSlab` layout; next is fd reconstruction/owner-death semantics and async intern integration shape.
...
**Next Action**: Continue 🔗[G110](./Knowledge/Goals/G110-EventfdEpollMicroVmIpcDesign/index.md) with fd reconstruction on resume, pidfd/owner-death recovery, cancellation/backpressure descriptor states, and the G091 async-intern integration decision. Then continue 🔗[G091](./Knowledge/Goals/G091-InternWorkerConcurrencyMvp/index.md) with broker-compatible `intern_start!` / `intern_sync!`; treat 🔗[G109](./Knowledge/Goals/G109-ListreeReadOnlyMutationSurfaceHardening/index.md) as the immediate safety companion before trusting shared read-only worker context.
