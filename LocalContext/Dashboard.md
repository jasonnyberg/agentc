...
1. 🔗[G117 — AgentC / DeltaGUI Market Data Hub Integration](./Knowledge/Goals/G117-DeltaGuiBackendToolAdapterExploration/index.md) — **IN PROGRESS**; Phases 0–3 complete and verified 2026-07-07: Phase 0 inventory/schemas (WP_G117), separate `markethub/` module with canonical events, pub/sub bus, quote cache, bounded history, price-move/stale-quote highlight engine, GreekScope GEX `extern "C"` façade (zero DeltaGUI modifications), and isolated TCC bridge demo through the G116 allowlist. Module-separation constraint honored: markethub consumes AgentC facilities and is never linked into core targets. Remaining: Phase 4 migration path (live-provider ingest, Edict-facing hub config, DeltaGUI subscriber migration).
...
### Blocked
No active blockers. Core TinyCC substrate goals G112-G116 are complete; G118 code-quality consolidation is complete. Latest validation (2026-07-07): `markethub_tests` 12/12 (TCC+façade build), `TccRuntimeTest.*` 12/12, full `./build/edict/edict_tests` 210/210, façade-disabled markethub build 9 passed/3 skipped/0 failed, and `AGENTC_ENABLE_TCC=OFF` markethub build 10 passed/2 skipped/0 failed.
...
- **G117 markethub module (Phases 0–3, 2026-07-07)**: new top-level `markethub/` module — standalone `markethub` static library (std-only: `MarketEvent`, `MarketHub` bus, `QuoteCache`, bucketed/bounded `HistoryStore`, `DeltaFeedSubscriber` since-sequence replay, `EnvelopeCollectorSubscriber` worker-envelope drain, `PriceMoveRule`/`StaleQuoteRule` highlight engine with injected clock, deterministic fixture ingest), plus `agentc_greekscope_facade` shared library compiled read-only from `~/DeltaGUI/backend_cpp` GEX sources (configurable via `AGENTC_MARKETHUB_DELTAGUI_ROOT`, auto-disabled when absent, zero DeltaGUI modifications). `markethub_tests` (12 tests) is the only target linking `libedict` — it drives the G116 allowlist + isolated TCC worker bridge demo returning real GEX snapshot JSON. Dependency direction is strictly markethub→AgentC; no core target references markethub. Phase 0 design record: WP_G117_MarketDataHubPhase0Inventory.
...
- **Incomplete backlog priority**: current order is G117 → G075. Core TinyCC goals G112-G116 are complete but not yet archived; G117 is in progress with Phases 0–3 implemented and verified — Phase 4 (migration path) is the remaining slice. G075 remains the long-arc speculative-architecture goal.
...
**Current State**: Core TinyCC integration goals G112-G116 are complete, and G118 consolidation is complete. G117 is in progress: Phases 0–3 are implemented and verified in the separate `markethub/` module — canonical event model, pub/sub hub with quote cache and bounded history, price-move/stale-quote highlight engine, two subscriber shapes, and a GreekScope GEX `extern "C"` façade invoked from isolated TCC workers through the G116 allowlist. DeltaGUI sources are consumed read-only; nothing in `~/DeltaGUI` was modified.

**Next Action**: G117 Phase 4 when requested: add a live-provider ingest adapter behind the native boundary, expose Edict-facing hub configuration/subscription words, and migrate DeltaGUI toward hub subscription. Read `markethub/README.md`, `markethub/include/markethub/market_hub.h`, and 🔗[WP — G117 Market Data Hub Phase 0 Inventory](./Knowledge/WorkProducts/WP_G117_MarketDataHubPhase0Inventory.md) first. Keep Cartographer/libffi and TinyCC as separate native paths, and avoid touching `~/DeltaGUI` unless a tiny non-breaking seam is genuinely required.

**Key Context**:
- **Latest verification (2026-07-07)**: `./build/markethub/markethub_tests` passes 12/12 (TCC + façade build); `./build/edict/edict_tests --gtest_filter=TccRuntimeTest.*` passes 12/12; full `./build/edict/edict_tests` passes 210/210; a façade-disabled markethub build passes 9 / skips 3; an `AGENTC_ENABLE_TCC=OFF` markethub build passes 10 / skips 2.
...
- 🔗[G117 — AgentC / DeltaGUI Market Data Hub Integration](./Knowledge/Goals/G117-DeltaGuiBackendToolAdapterExploration/index.md) — in progress; Phases 0–3 implemented and verified in the separate `markethub/` module (hub MVP, highlight engine, GEX façade + isolated TCC bridge demo); Phase 4 migration path remains.
