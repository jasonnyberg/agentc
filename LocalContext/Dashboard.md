...
- **Current Task**: Define and implement the first explicit transient rehydration boundary for host/runtime artifacts around the new embedded VM/root-anchor restore path.
- **Immediate Next Action**: Formalize how runtime handles/imports are rebuilt around embedded VM restore, then continue shrinking the remaining host-owned JSON mutation path.
...
- The persistence demo path was also cleaned up: `cpp-agent/demo/demo_runtime_persistence.sh` now runs reliably under zsh and no longer collides with the ambient `HOST` environment variable while demonstrating `Restored persisted agent root ...` on restart, even when the provider returns transient high-demand errors.
