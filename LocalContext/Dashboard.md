...
**Current State**: G111 is complete; G091 has first retained-terminal/drop/abandoned/non-retroactive-cancel lifecycle semantics, G099 has first opt-in Edict task/status validators, and G110 now has known-resource abandoned-owner recovery, deterministic lease/expiry recovery for registered resources, and non-parking module-backed `root1.await!`.

**Next Action**: Continue 🔗[G110](./Knowledge/Goals/G110-EventfdEpollMicroVmIpcDesign/index.md) by defining how participant heartbeats and pidfd owner-death events update Root1 lease records, then connect expired/pid-dead lease recovery to process-worker ownership policy; start in `core/root1_resource_broker.{h,cpp}` and extend `tests/root1_resource_broker_tests.cpp`.
...
- Current G110 abandoned recovery has two layers: explicit `recoverAbandonedResource(key, state, owner, reason)` and lease-driven `registerLease` / `renewLease` / `recoverExpiredLeases(nowTick, reason)` for registered known resources.
