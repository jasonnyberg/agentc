...
**Active Task**: Continue G049 follow-through with focused G050 verification. The mixed-run spawn/join issue now has candidate fixes landed (helper target dependencies plus atomic slab-handle retain), and the next task is broader completion-level validation beyond the restored repeated focused thread-runtime runs.
...
1. **Finish G050 Verification**: Re-establish a completed standalone full `./build/edict/edict_tests` run and full `ctest` after the helper-dependency and atomic-retain fixes.
