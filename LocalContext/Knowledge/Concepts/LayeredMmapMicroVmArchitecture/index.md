...
3. Shared readonly core slabs — worker maps imports/modules instead of copying them.
4. Published result slabs — worker publishes immutable result roots; coordinator imports/merges by reference or copies on write.
5. Root0-built static core image — the bootstrap/root0 VM saves core slabs and retires.
6. Root1 coordinator — a micro-VM mounted over the static core owns the inter-VM messaging tree and slab-advertisement registry.
7. Process-isolated micro-VM cores — many tiny intern processes map the same core image and own only private overlays or leased publication slabs.
...
- G092 should classify imports not just as safe/unsafe, but also as static-shareable, per-process-rehydratable, thread-safe, process-safe, or VM-private.
