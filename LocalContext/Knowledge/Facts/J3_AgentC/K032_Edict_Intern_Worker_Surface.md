...
1. Build on G110's first Root1 broker prototype (`ResourceKey`, participant eventfds, mailbox descriptors, bounded ring, grant events) to define logical job waitable ids, event kinds, ownership/error states, backpressure/cancel states, and future publication handles before committing to the async backend.
