...
## Current Limits
- `intern_start!` / `intern_sync!` / `intern_cancel!` are implemented, and completed G111 provides a module-backed path using imported worker primitives; module-backed `intern_run!` composes prepare/run-status/Edict run-envelope, `intern_start!` composes prepare/capacity-status/Edict backpressure-envelope/start-status/Edict start-envelope, and `intern_sync!` / `intern_cancel!` compose drain-or-request-cancel, collect-status facts, and Edict-owned public sync envelopes. The intern VM opcode shims and legacy native public-envelope service paths have been removed; deeper lifecycle policy now belongs to G091/G110 follow-ups.
