...
The purpose is to prove the safe coordinator/worker boundary before introducing live local-model interns or a larger scheduler. 🔗[G111 — Root1/Worker Primitive FFI and Edict Intern Surface Migration](../../Goals/G111-Root1WorkerPrimitiveFfiEdictInternMigration/index.md) completed the migration to expose only irreducibly native Root1/worker mechanisms through importable libraries and move intern policy/envelopes into Edict modules. The G111/G099/G091 slices added importable LTV worker primitives plus `worker.edict` / `intern.edict`; module-backed `intern_run!` composes prepare/run-status/Edict run-envelope, module-backed `intern_start!` composes start-status/Edict start-envelope with native dispatch-bound contract checks, module-backed `intern_sync!` composes drain/collect-status/Edict sync-envelope words, module-backed `intern_cancel!` composes request-cancel/collect-status/Edict sync-envelope words, `worker.edict_active_count!` exposes job-table pressure, `worker.edict_lifecycle_status!` exposes process-local lifecycle accounting, `worker.edict_drop!` provides explicit cleanup/abandonment, `worker.edict_validate_result_contract!` backs explicit result trust checks, first `root1.edict` participant/poll/mailbox wrappers are importable, the native implementation is now split across `edict_worker_runtime.{h,cpp}`, `edict_worker_primitives.cpp`, and thin C ABI wrappers in `edict_worker_primitives_ffi.cpp`, legacy native run/start/collect/sync/cancel envelope paths have been deleted, and the intern VM opcode shims have been deleted.
...
`intern_start!` returns a job handle with `job_id`, `state: "started"`, a broker-shaped `waitable`, and reserved `publication: null`. `intern_sync!` returns `state: "running"` or `state: "cancel_requested"` until the worker completes, then returns the same structured final result shape as `intern_run!` plus `job_id`, `waitable`, and broker descriptor `events`. `intern_cancel!` is a module-backed Edict word over request-cancel/collect-status primitives; it requests cooperative cancellation and final sync reports `state: "cancelled"` without merging the worker result.
...
- lifecycle accounting for active/tracked/retained-terminal/abandoned-running jobs and cumulative started/finished/abandoned/dropped/swept counters

Latest focused validation: `./build/edict/edict_tests --gtest_filter='InternWorkerTest.*:Root1AwaitSchedulerTest.*:Root1PrimitiveModuleTest.*:EdictVM.YieldedExecutionCanResumeCurrentCodeFrame' --gtest_brief=1` passed 21/21 on 2026-05-19, including async `intern_start!` / `intern_sync!`, `intern_cancel!`, backpressure, unknown-job coverage, the module-backed FFI worker-primitive coverage, direct lower-level worker prepare-task/capacity-status/run-status/start-status/drain/collect-status/request-cancel/result-contract/lifecycle-status coverage, confidence/evidence-threshold result rejection, module-level Edict-owned run/start/backpressure/sync/cancel envelope branching, active-count coverage, explicit drop cleanup/abandonment accounting coverage, first module-backed Root1 participant/poll/mailbox descriptor coverage, and proof that raw VM startup no longer installs intern words as bootstrap builtins.
...
- `intern_cancel!` is implemented as plain Edict composition (bootstrap compatibility word and module-backed request-cancel/collect-status path); it marks an active job as cooperatively cancelled, emits a `Cancelled` descriptor, and causes final sync to return `state: "cancelled"`; this first slice does not preemptively kill the detached worker thread.
...
## Current Limits
- `intern_start!` / `intern_sync!` / `intern_cancel!` are implemented, and completed G111 provides a module-backed path using imported worker primitives; module-backed `intern_run!` composes prepare/run-status/Edict run-envelope, `intern_start!` composes start-status/Edict start-envelope with dispatch-bound contract checks, and `intern_sync!` / `intern_cancel!` compose drain-or-request-cancel, collect-status facts, and Edict-owned public sync envelopes. The intern VM opcode shims and legacy native public-envelope service paths have been removed; deeper lifecycle policy now belongs to G091/G110 follow-ups.
- Cancellation is cooperative-only and non-retroactive: if the request is accepted before terminal completion, final sync returns `cancelled`; if the worker is already terminal, cancellation is a no-op and the complete/error result remains visible. Terminal async jobs are retained for repeated sync until explicit `worker.edict_drop!` or bounded sweep; dropping a running job abandons the handle and suppresses later orphan completion publication, while dropping a terminal job forgets the retained result. Explicit worker arena lifecycle cleanup and process-isolated workers remain future work.
