...
HOST="${HOST:-$PROJECT_ROOT/build/cpp-agent/cpp-agent}"
...
"$HOST" --config "$CONFIG" --socket "$SOCKET" --state-base "$STATE_BASE" >"$LOG1" 2>&1 & echo $! > "$PID1"
sleep 1
python3 - <<PY
import socket, time
...
"$HOST" --config "$CONFIG" --socket "$SOCKET" --state-base "$STATE_BASE" >"$LOG2" 2>&1 & echo $! > "$PID2"
sleep 1
python3 - <<PY
import socket, time
sock = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
